...
export const autentica = (req,res,next) => {
    ...
    if(!token){
        res.status(401).json({ok: false, error: 'token absent', status: 401})
        // return;
    }
    ...
    jsonwebtoken.verify(token, secretKey, (error,decoded) =>{
        ...
        if(error){
            res.status(401).json({ok: false, error: 'token invalid', status: 401});
            ...
            if (expiredAt > new Date().getTime()){
                ...
                res.status(401).json({ok: false, error: 'token caducat', status: 401});
            }
        }
    });
};